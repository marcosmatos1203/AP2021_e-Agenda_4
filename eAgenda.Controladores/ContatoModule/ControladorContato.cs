...
namespace eAgenda.Controladores.ContatoModule
{
    public class ControladorContato : Controlador<Contato>
    {
        ...
        private const string sqlSelecionarContatoPorNome =
            @"SELECT
                        [ID],
		                [NOME], 
		                [EMAIL], 
        ...
        private const string sqlSelecionarTodosContatos =
            @"SELECT
		                ...
		                [EMPRESA]
	                FROM
                        TBCONTATO ORDER BY NOME;";

        private const string sqlSelecionarTodosContatosAgrupadoPorCargo =
           @"SELECT
                        [ID],
		                [NOME], 
		                [EMAIL], 
        ...
        private const string sqlSelecionarTodosContatosAgrupadoPorId =
           @"SELECT
                        [ID],
		                [NOME], 
		                [EMAIL], 
        ...
        public Contato SelecionarPorNome(string nome)
        {
            return Db.Get(sqlSelecionarContatoPorNome, ConverterEmContato, AdicionarParametro("NOME", nome));
        }
        public override List<Contato> SelecionarTodos()
        {
            return Db.GetAll(sqlSelecionarTodosContatos, ConverterEmContato);
        }
        public List<Contato> SelecionarTodosContatosAgrupadoPorCargo()
        {
            return Db.GetAll(sqlSelecionarTodosContatosAgrupadoPorCargo, ConverterEmContato);
        }
        public List<Contato> SelecionarTodosContatosAgrupadoPorId()
        {
            return Db.GetAll(sqlSelecionarTodosContatosAgrupadoPorId, ConverterEmContato);
        }
